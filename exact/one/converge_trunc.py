from onetransmon.hamil import calc_eigs, plt
import numpy as np
import cupy as cp
...
# We try different Ej
kk = np.concatenate(
    [np.arange(start=6, stop=20, step=1), np.arange(start=22, stop=50, step=4)]
)
...
def run(Ej):
    ...
    plt.suptitle(
        f"Convergence of energy levels, Ej={Ej}, dashed line is low_cut={kk[low_cut]}"
    )
