...
def decomp():
    ...
    c1 = ax1.pcolormesh(d2prim_grid, dd13_grid, vals, norm=norm, cmap=cmap, rasterized=True)
    # ax1.set_title("ZZZ")

    c2 = ax2.pcolormesh(d2prim_grid, dd13_grid, vals01, norm=norm, cmap=cmap, rasterized=True)
    # ax2.set_title("01")

    c3 = ax3.pcolormesh(d2prim_grid, dd13_grid, vals12, norm=norm, cmap=cmap, rasterized=True)
    # ax3.set_title("12")

    c4 = ax4.pcolormesh(d2prim_grid, dd13_grid, vals02, norm=norm, cmap=cmap, rasterized=True)
    # ax4.set_title("02")

    c5 = ax5.pcolormesh(d2prim_grid, dd13_grid, vals3, norm=norm, cmap=cmap, rasterized=True)
    # ax5.set_title("3loops")

    c6 = ax6.pcolormesh(d2prim_grid, dd13_grid, vals4, norm=norm, cmap=cmap, rasterized=True)
    # ax6.set_title("Residual 4-contractions")

    cbar = fig.colorbar(c1, ax=[ax1, ax2, ax3, ax4, ax5, ax6], fraction=0.05)
    cbar.set_label(r"ZZZ [$-\alpha$]")
    ...
    fig.savefig("figs/swt-zzz-recreate-decomp.pdf", dpi=300, bbox_inches="tight")
    plt.show()
