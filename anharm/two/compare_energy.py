...
def sw_energies(Ej1: np.ndarray, Ej2: float, Eint):
    ...
    Ham = Hamil(2, 3, "line")
    e10: sp.Expr = Ham.get_all("10", keep_second_coupling=keep)
    e01: sp.Expr = Ham.get_all("01", keep_second_coupling=keep)
    e11: sp.Expr = Ham.get_all("11", keep_second_coupling=keep)
    e20: sp.Expr = Ham.get_all("20", keep_second_coupling=keep)
    e02: sp.Expr = Ham.get_all("02", keep_second_coupling=keep)
...
# plt.plot(Ej1, omega2 * np.ones(omega1s.shape), lw=0, marker=".", label="01")
# plt.plot(Ej1, (2 * omega2 + alpha2) * np.ones(omega1s.shape), lw=0, marker=".", label="02")
